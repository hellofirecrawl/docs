---
title: 'Compare (Beta)'
description: 'Firecrawl can compare the current page to a previous version, and tell you if it changed or not'
og:title: "Compare (Beta) | Firecrawl"
icon: "code-compare"
og:description: "Firecrawl can compare the current page to a previous version, and tell you if it changed or not"
---

Using the new `compare` format, you can monitor changes on a website.

The `compare` format returns the following fields in the response:
...
- `visibility`: The visibility of the current page/URL.
    ...
    - `invisible`: This page is not visible, meaning that its URL was discovered through memory from previous crawls of the same URL.

Some good to know facts:
- The scrapes are always compared via their markdown response.
    - The `markdown` format must also be specified when using the `compare` format. Other formats may also be specified in addition.
...
- Previous scrapes to compare against are currently only matched on the source URL, the team ID, and the `markdown` format, and not on any other scrape or crawl options.
    ...
    - Crawling the same URLs with different `includePaths`/`excludePaths` will have inconsistencies when using `compare`.
    - Scraping the same URLs with different `includeTags`/`excludeTags`/`onlyMainContent` will have inconsistencies when using `compare`.
    - Compared pages will also be compared against previous scrapes that only have the `markdown` format without the `compare` format.
...
- While in Beta, it is recommended to monitor the `warning` field of the resulting document, and to handle the `compare` object potentially missing from the response. We will be monitoring the system to ensure this happens as little as possible.
