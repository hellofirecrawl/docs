...
- **allowExternalLinks** (Optional): When `true`, extraction can follow links outside the specified domain.
...
Setting `allowExternalLinks = true` in your request will expand the crawl beyond the provided URL set. This can capture supporting or related information from linked pages.
