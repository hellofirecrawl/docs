...
Extract structured data from a single or multiple URLs using Large Language Models (LLMs). This endpoint is ideal for:
- Extracting structured data from websites
- Useful for data enrichment applications that need to structured data from websites
- Developing AI applications that need clean data from multiple websites
...
Used to extract structured data from multiple URLs. When specifying URLs, you can append `/*` to extract information from the entire website rather than just a single page. For example, `https://firecrawl.dev/*` will attempt to extract data from all pages on the firecrawl.dev domain. The `/*` is still in beta so please let us know if you have any issues by emailing help@firecrawl.dev.
